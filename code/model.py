...
class ISD(nn.Module):
    def __init__(self, K=48, m=0.99, Ts=0.1, Tt = 0.01, num_classes=4, train_encoder=True, train_decoder=True, 
                latent_pooling_size=1, latent_feature_size=256, output_pooling_size=16, patch_size=64): # K=48
    ...
    def forward(self, im_q, im_k):
        ...
        output_tea_after_head = torch.cat(output_tea_after_head).reshape(batch_size, -1, 
                                        ...
                                        output_tea_after_head[0].shape[3]).contiguous() 
...
if __name__ == '__main__':
    ...
    model = ISD(K=24, m=0.99, Ts=0.07, Tt=0.01, num_classes=4, train_encoder=True, train_decoder=True, 
                latent_pooling_size=1, latent_feature_size=512, output_pooling_size=8, patch_size=64).cuda()
    model.data_parallel()
    input = torch.ones([6, 1, 256, 256]).cuda()
    for i in range(5):
        outputs, ema_output, ema_latent_logits, latent_logits, ema_output_logits, output_logits= model(input, input)
        # exit()
        print(outputs.shape)