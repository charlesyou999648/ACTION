...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        output, feature_map = self.decoder(feature)
        return output, feature[-1], feature_map
...
if __name__ == '__main__':
    net = UNet(in_chns=3, class_num=2, train_encoder=False, train_decoder=True)
    img = torch.ones([4, 3, 256, 256])
    output, feature, feature_map = net(img)
    # print(output.shape) # torch.Size([4, 2, 256, 256])
    # print(feature.shape) # torch.Size([4, 256, 16, 16])
