...
parser.add_argument('--k1', type=float, default=0.01, help='the weights for latent contrastive loss')
...
parser.add_argument('--temp_tsc', default=0.07, type=float, help='low')
parser.add_argument('--temp_low', default=0.1, type=float, help='low')
parser.add_argument('--T', type=float, default=0.1, help='T-> period', choices=[0.1, 0.2, 0.5, 1.0, 2.0])
parser.add_argument('--lambda_tsc', type=float, default=0.1, help='T-> period', choices=[0.05, 0.1, 0.2, 0.5, 1.0])
...
def train(args, snapshot_path):
    ...
    isd = ISD(K=args.K, m=0.99, Ts=0, Tt=0, num_classes=num_classes, latent_pooling_size=args.latent_pooling_size, 
    ...
    q_representation = RepresentationHead(num_classes=256+128+64+32+16, output_channel=496).cuda()
    isd.train()
    model.train()
    ema_model.train()
    ...
    tsc = TSC(num_class=num_classes, targeted=True, \
        random=True, T=args.temp_tsc, feature_size=496, K=256*3*4*4, tw=args.lambda_tsc).cuda()
    ...
    for epoch_num in iterator:
        ...
        temp = get_current_t(epoch_num, max_epoch, \
            T_low=args.temp_low, T_high=args.temp_high, T=args.T)
        tsc.T = get_current_t(epoch_num, max_epoch, \
            T_low=args.temp_tsc, T_high=args.temp_high, T=args.T)
        ...
        for i in range(len(train_u_loader)):
            ...
            _, _, l_feature_map_t = ema_model(train_l_data)
            pred_l, _, l_feature_map = model(train_l_data)
            pred_u, _, u_feature_map = model(train_u_aug_data)

            for i in range(len(l_feature_map)):
                l_feature_map[i] = F.interpolate(l_feature_map[i], size=pred_l.shape[2:], mode='bilinear', align_corners=True)
                u_feature_map[i] = F.interpolate(u_feature_map[i], size=pred_l.shape[2:], mode='bilinear', align_corners=True)
                l_feature_map_t[i] = F.interpolate(l_feature_map_t[i], size=pred_l.shape[2:], mode='bilinear', align_corners=True)
                
            l_feature_all = torch.concat(l_feature_map, dim=1)
            u_feature_all = torch.concat(u_feature_map, dim=1)
            l_feature_map_t = torch.concat(l_feature_map_t, dim=1)
            ...
            reco_loss = compute_reco_loss(rep_all, label_all, mask_all, prob_all, args.strong_threshold,
                                          temp, args.num_queries, args.num_negatives)
            
            tsc_loss = tsc(pred_l, rep_l, l_feature_map_t, train_l_label.long())

            loss = 0.01*reco_loss + unsup_loss + supervised_loss + 0.01*tsc_loss
            ...
            writer.add_scalar('info/tsc_loss', tsc_loss, iter_num)
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, reco_loss: %f, unsup_loss: %f, tsc_loss: %f' %
                (iter_num, loss.item(), loss_ce.item(), loss_dice.item(), reco_loss.item(), unsup_loss.item(), tsc_loss.item()))
