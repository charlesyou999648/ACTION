...
def train(args, snapshot_path):
    ...
    k_representation = RepresentationHead_3d(num_classes=128+64+32+16+16, output_channel=128).cuda()
    ...
    model.train()
    ema_model.train()
    k_representation.train()
    q_representation.train()
    
    with torch.no_grad():
        for t_params, s_params in zip(k_representation.parameters(), q_representation.parameters()):
    ...
    tsc = TSC_3d(num_class=num_classes, targeted=True, random=True, T=0.07, feature_size=128, K=256*3*4*4).cuda()
    ...
    for epoch_num in iterator:
        ...
        for i in range(len(train_u_loader)):
            ...
            for i in range(len(l_feature_map)):
                ...
                l_feature_map_t[i] = F.interpolate(l_feature_map_t[i], size=pred_l.shape[-3:], mode='trilinear', align_corners=True)
            ...
            l_feature_map_t = k_representation(l_feature_map_t)

            rep_all = torch.cat((rep_l, rep_u))
            # print(torch.unique(rep_all))
            ...
            with torch.no_grad():
                for param_q, param_k in zip(q_representation.parameters(), k_representation.parameters()):
...
def compute_unsupervised_loss(predict, target, logits, strong_threshold):
    ...
    # print('predict', predict.shape) # predict torch.Size([2, 2, 112, 112, 80])
    # print('target', target.shape) # target torch.Size([224, 112, 80])
