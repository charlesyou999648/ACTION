...
parser.add_argument('--k5', type=float, default=1.0, help='uamt loss')
...
def train(args, snapshot_path):
    ...
    model = isd.model
    q_representation = RepresentationHead(num_classes=256+128+64+32+16, output_channel=512).cuda()
    params = [p for p in model.parameters() if p.requires_grad]
    params_repr = [p for p in q_representation.parameters() if p.requires_grad]
    optimizer = optim.SGD(params+params_repr, lr=base_lr, weight_decay=0.0001, momentum=0.9, nesterov=True)
    isd.train()
    model.train()
    ema_model.train()
    ...
    for epoch_num in iterator:
        ...
        for i in range(len(train_u_loader)):
            ...
            pred_u, _ , second_to_last = ema_model(train_u_data)
            ...
            train_u_aug_data, train_u_aug_label, train_u_aug_logits = \
                batch_transform(train_u_aug_data, train_u_aug_label, logits=train_u_aug_logits, scale_size=(1, 1), apply_augmentation=True)
            ...
            outputs, ema_output, = isd(train_u_data, train_u_aug_data)[:2]

            loss = args.k1*reco_loss + unsup_loss + supervised_loss 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
